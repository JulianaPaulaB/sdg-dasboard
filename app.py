...
with st.sidebar:
    ...
    # Generate Fully Dynamic Sidebar Status Tags From Model Data
    def dynamic_sidebar_status(feature_name):
        coef = rlm_model.params[feature_name]
        p_val = rlm_model.pvalues[feature_name]
        direction = "Positive" if coef > 0 else "Negative"
        p_str = "p&lt;0.001" if p_val < 0.001 else f"p={p_val:.3f}"
        
        if p_val < 0.05:
            color = GREEN if coef > 0 else RED
            return f"<span style='color:{color};margin-left:12px'>● {direction} ({p_str})</span>"
        else:
            return f"<span style='color:{GRAY};margin-left:12px'>● Not significant ({p_str})</span>"
    ...
    <div style="font-size:11px;line-height:1.8;color:rgba(255,255,255,0.65);">
        ...
        {dynamic_sidebar_status("Healthcare_Spending")}<br>
        💧 Water Access<br>
        {dynamic_sidebar_status("Water_Access")}<br>
        💰 GDP per Capita<br>
        {dynamic_sidebar_status("GDP_per_capita")}<br>
        🌫️ CO₂ Emissions<br>
        {dynamic_sidebar_status("CO2_Emissions")}<br><br>
...
factor_cfg = [
    (f1, "Healthcare_Spending", "Life_Expectancy", "Healthcare Spending (USD)",
     ...
     "Positive relationship — Higher cumulative healthcare investment is structurally paired with longer life expectancy trends. (Jaba et al., 2014)"),
    (f2, "GDP_per_capita", "Life_Expectancy", "GDP per Capita (USD)",
     "Life Expectancy (yrs)", "💰 GDP per Capita", "amber",
     "Positive relationship — Wealthier historical baselines allow nations to afford higher standards of overall care. (Preston, 1975)"),
    (f3, "Water_Access", "Life_Expectancy", "Water Access (%)",
     "Life Expectancy (yrs)", "💧 Water Access", "green",
     "Positive relationship — High clean water infrastructure limits the public burden of waterborne vectors. (UN SDG Report, 2023)"),
    (f4, "CO2_Emissions", "Life_Expectancy", "CO₂ Emissions (tons/capita)",
     "Life Expectancy (yrs)", "🌫️ CO₂ Emissions", "red",
     "Negative relationship — High pollution levels run adverse to prolonged structural respiratory well-being. (Wen et al., 2021)"),
]

for col_widget, x_col, y_col, x_lbl, y_lbl, title, box_cls, insight in factor_cfg:
    with col_widget:
        # Fixed: Trendlines removed from dynamic frames to bypass Plotly Express ValueError exceptions
        fig_sc = px.scatter(
...
with col_corr:
    ...
    for feat, val in corr_with_le.items():
        ...
        # Enhanced Direction Text Range Classification Boundaries
        if val > 0:
            direction = "strong +" if val > 0.7 else "moderate +" if val > 0.3 else "weak +"
        else:
            direction = "strong −" if val < -0.7 else "moderate −" if val < -0.3 else "weak −"
    ...
    # Fully Dynamic Insight Extraction Strings
    c_co2 = corr_matrix.loc["Life_Expectancy", "CO2_Emissions"]
    c_h2o = corr_matrix.loc["Life_Expectancy", "Water_Access"]
    c_hlth = corr_matrix.loc["Life_Expectancy", "Healthcare_Spending"]
    st.markdown(f"""<div class='insight-box' style='margin-top:12px'>
        CO₂ Emissions maintains a correlation of {c_co2:+.2f} with Life Expectancy, 
        followed closely by Water Access ({c_h2o:+.2f}) and Healthcare Spending ({c_hlth:+.2f}). 
        The inverse spending correlation reflects cross-country structural trade-offs: the UK sustains lower absolute health spending per citizen yet achieves superior health outcomes relative to the US dataset profile.
...
with col_perf:
    ...
    mc[2].metric("Observations", f"{len(dff_pred)}",           "Active Data Scope")
...
with col_fi:
    # Fully Dynamic Coefficient Extraction Array Generation
    coef_df = pd.DataFrame({
        "Variable": ["Healthcare Spending", "Water Access", "GDP per Capita", "CO₂ Emissions"],
        "Coefficient": [
            rlm_model.params["Healthcare_Spending"],
            rlm_model.params["Water_Access"],
            rlm_model.params["GDP_per_capita"],
            rlm_model.params["CO2_Emissions"]
        ],
        "p-value": [
            rlm_model.pvalues["Healthcare_Spending"],
            rlm_model.pvalues["Water_Access"],
            rlm_model.pvalues["GDP_per_capita"],
            rlm_model.pvalues["CO2_Emissions"]
        ]
    })
    
    coef_df["Significant"] = coef_df["p-value"].apply(
        lambda p: "Yes (p<0.001)" if p < 0.001 else ("Yes (p<0.05)" if p < 0.05 else f"No (p={p:.2f})")
    )
    ...
    fig_fi = go.Figure(go.Bar(
        ...
        marker_line_width=0,
        text=coef_df["p-value"].apply(lambda p: "p<0.001" if p < 0.001 else f"p={p:.4f}"),
    ...
    # Modernized style.map implementation replacing deprecated applymap methods
    st.dataframe(
        coef_df.style.map(
            lambda v: f"color:{GREEN};font-weight:600" if "Yes" in str(v) else f"color:{GRAY}",
            subset=["Significant"]
        ).map(
...
<div class="conclusion-box">
    ...
    <p>
        ...
        <strong>Healthcare Spending</strong> is a statistically significant driver of life expectancy (p&lt;0.05),
        while <strong>CO₂ Emissions</strong> is a significant negative driver (p&lt;0.001).
        <strong>GDP per Capita</strong> and <strong>Water Access</strong> coefficients within this high-collinearity profile serve as vital comparative controls, demonstrating that their structural effects are often mediated jointly when analyzing highly advanced nations.
