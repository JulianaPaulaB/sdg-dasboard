...
<div class="academic-header">
    ...
    <div class="academic-quote">
        ...
        This comprehensive dashboard investigates the multifaceted drivers of life expectancy using 24 years of real-world macroeconomic and public health data from the World Bank and Our World in Data. We apply Multiple Linear Regression and Robust Regression (RLM) frameworks to isolate and identify exactly which socio-economic and environmental variables significantly explain the variation in human longevity.
    </div>
</div>
""", unsafe_allow_html=True)
...
def format_delta(delta, format_type="standard"):
    if prev_data.empty: 
...
<div class="kpi-container">
    ...
    <div class="kpi-card" style="border-top: 4px solid #3b82f6;">
        ...
        {format_delta(delta_health, "currency")}
    </div>
    <div class="kpi-card" style="border-top: 4px solid {C_ORANGE};">
        ...
        {format_delta(delta_gdp, "currency")}
    </div>
...
if show_all:
    ...
    with r1:
        ...
        for country in chart_df['Country Name'].unique():
            ...
            fig_pred.add_trace(go.Scatter(x=c_df['Year'], y=c_df['Life Expectancy'], mode='lines+markers', name=f'{country} Actual', line=dict(color=color, width=3)))
            fig_pred.add_trace(go.Scatter(x=c_df['Year'], y=c_df['Predicted'], mode='lines', name=f'{country} Predicted', line=dict(color=color, width=3, dash='dash')))
